...
export default function LatestPage() {
  ...
  const refreshLatest = () => {
    setFetching(true);
    setPage(1);
    feedApi.latest(1, true)  // ← Now triggers backend to fetch fresh content
      .then((r) => setArticles(mergeUniqueById([], r.articles)))
      .catch((e) => setError(e.message))
      .finally(() => setFetching(false));
  };
    ...
    <div>
      <div className="mb-8 reveal rounded-3xl border border-black/15 bg-white/75 backdrop-blur px-4 py-4 sm:px-6 sm:py-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-3xl text-black">Latest</h1>
              <p className="text-sm text-black/60">Most recent stories flowing in from all sources</p>
            </div>
          </div>
          <button
      ...
      {!fetching && !error && articles.length === 0 && (
        <div className="glass rounded-3xl mt-8 min-h-[224px] px-6 py-12 flex flex-col items-center justify-center text-center space-y-3">
          <p className="font-display text-2xl text-black">No fresh articles right now</p>
          <p className="text-black/55 text-sm">Try refresh in a few moments.</p>
        </div>
      )}
